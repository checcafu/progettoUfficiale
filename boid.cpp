...
int main() {
  ...
  std::cout << "Insert the number of boids (a whole number between 2 and 250)"
            << '\n';
  try {
    int n;
    ...
    for (int t = 0; t < 660; t++)  // ciclo for che simula tempo (qui va da 0s a
                                   // 10s, è provvisorio come valore per adesso)
    {
      if (t % 60 == 0) {
        std::cout << '\n'
                  << "Time:" << t / 60 << " "
                  << "min" << '\n';
        std::cout << "Mean speed:" << simulation.meanspeed() << "+/-"
                  << simulation.stdmeanspeed() << '\n';
        std::cout << "Mean distance:" << simulation.meandistance() << "+/-"
                  << simulation.stdmeandistance() << '\n';
      }
